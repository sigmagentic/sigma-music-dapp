...
- review and fix any worng info in FAQ

-- dynomobd

- create all the new tables and indexes -- DONE
- make all the streams -- DONE

-- lambda

- copy the new lambdas and the roles etc -- DONE

-- cloud

- new S3 buckets -- DONE
- new SQS queues? -- DONE (NOT NEEDED)
- we made some cloudfront changes, make those too -- DONE

-- story

- create the new licence formats on mainnet for ccby 4 and the royalty one (that's a future version)
- update our API references to this.
- update the app references?? (if needed)

-- api

- finally, push the API to live

-- S3 Changes
we tightended the S3 public bucket -- Bucket Polcy on the assestpublic was this:
{
"Version": "2012-10-17",
"Statement": [
{
...
"Principal": "*",
"Action": "s3:GetObject",
"Resource": "arn:aws:s3:::assetspublic-itheum-ecosystem/*"
}
]
}
...
"Action": "s3:GetObject",
"Resource": "arn:aws:s3:::assetspublic-itheum-ecosystem/_",
"Condition": {
"StringEquals": {
"AWS:SourceArn": "arn:aws:cloudfront::028648833949:distribution/E26Q3PR4TAS1R5"
}
}
},
{
...
"Effect": "Allow",
"Principal": {
"AWS": "arn:aws:iam::028648833949:role/datadexapisls-prd-eu-central-1-lambdaRole"
},
"Action": [
"s3:PutObject",
"s3:PutObjectAcl",
