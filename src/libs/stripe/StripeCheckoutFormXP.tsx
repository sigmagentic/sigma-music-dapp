...
type StripeCheckoutFormXPProps = {
  priceInUSD: string | null;
  commitedXPOption: string | null;
  XPCollectionIdToUse: string;
  closeStripePaymentPopup: () => void;
};

const StripeCheckoutFormXP = ({ priceInUSD, closeStripePaymentPopup, commitedXPOption, XPCollectionIdToUse }: StripeCheckoutFormXPProps) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const XPPurchasedFromUrl = encodeURIComponent(location.pathname + location.search);

      const { error: submitError } = await stripe.confirmPayment({
        elements,
        confirmParams: {
          return_url: `${window.location.origin}/payment-success?buyerSolAddress=${buyerSolAddress}&priceInUSD=${priceInUSD}&billingEmail=${encodeURIComponent(email)}&XPPurchase=true&XPPurchasedFromUrl=${XPPurchasedFromUrl}&XPBeingBought=${commitedXPOption}&XPCollectionIdToUse=${XPCollectionIdToUse}`,
          receipt_email: email,
        },
      });
...
export default StripeCheckoutFormXP;
